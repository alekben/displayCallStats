...
var newice = true;
console.log("Setting new ice restart at start");
...
$("#join-form").submit(async function (e) {
  ...
  $("#setIceRestart").attr("disabled", true);
  try {
    if (!client) {
...
$("#setIceRestart").click(function (e) {
  if (newice) {
    $("#setIceRestart").text("Enable New Ice Restart");
    newice = false;
    AgoraRTC.setParameter('NEW_ICE_RESTART', false);
    var x = document.getElementById("popup");
    $("#popup").text(`New Ice Restart disabled`);
    x.className = "show";
    setTimeout(function(){ x.className = x.className.replace("show", ""); }, 3000);
  } else {
    $("#setIceRestart").text("Disable New Ice Restart");
    newice = true
    AgoraRTC.setParameter('NEW_ICE_RESTART', true);
    var x = document.getElementById("popup");
    $("#popup").text(`New Ice Restart enabled`);
    x.className = "show";
    setTimeout(function(){ x.className = x.className.replace("show", ""); }, 3000);
  }
});
...
async function leave() {
  ...
  $("#setIceRestart").attr("disabled", false);
