...
$(() => {
  ...
  options.proxy = urlParams.get("proxy");
  if (options.proxy == null) {
    options.proxy = 0
  };
  mode.value = options.proxy;
  if (options.proxy == 3) {
    mode.label = "UDP Mode";
    mode.detail = "Enable Cloud Proxy via UDP protocol";
  };

  if (options.proxy == 5) {
    mode.label = "TCP Mode";
    mode.detail = "Enable Cloud Proxy via TCP/TLS port 443";
  };
...
$("#join-form").submit(async function (e) {
  ...
  try {
    ...
    options.uid = $("#uid").val();
    options.uid2 = (options.uid + "_recv");
...
async function join() {
  ...
  options.uid2 = await client2.join(options.appid, options.channel, options.token || null, options.uid2);
...
async function leave() {
  ...
  $("#areaButton").attr("disabled", false);
...
async function changeRemoteStream() {
  if (lowStream) {
    lowStream = false;
    client2.setRemoteVideoStreamType(options.uid, 0);
  } else {
    lowStream = true;
    client2.setRemoteVideoStreamType(options.uid, 1);
  }
}
