...
async function pickS() {
  ...
  if (slayer == 3) {
    $("#pickSLayer").text("S2");
    slayer = 2;
    let SVCString = `"${id}", {spatialLayer:Number${slayer},temporalLayer:${tlayer}}`;
    console.log(SVCString);
    client.pickSVCLayer(
      {uid: `${id}`, layerOptions: {spatialLayer: `${layer}`, temporalLayer: `${tlayer}`}}
      );
  }
  else if (slayer == 2) {
    $("#pickSLayer").text("S1");
...
async function join() {
  ...
  if (!localTracks.videoTrack) {
    localTracks.videoTrack = await AgoraRTC.createCameraVideoTrack({encoderConfig: {width: 1920, height: 1080, frameRate: 30, bitrateMax: 3000, bitrateMin:750}, scalabilityMode: "3SL3TL"});
  }
